...
def return_files(class_x, next_class=False):
    ...
    for (i, f) in enumerate(files):
        ...
        if(i == 0):
            ...
            files_ar = np.concatenate((files_ar, np.array([img])))
    
    # PCA
    files_arr = files_ar
    for files_ar in files_arr:
        ...
        files_ar_t = pca_model_r.inverse_transform(pca_out_r)
        cv2.imwrite('tmp.jpg', files_ar_t)
        input()
    print('files_ar_t.shape: ', files_ar_t.shape)
